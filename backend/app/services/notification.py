"""Create in-app notifications and enqueue emails for order events."""
...
def fan_out_forwarded(db: Session, order: Order) -> None:
    ...
    _send_emails_bg(
        ...
        body_html=(
            ...
            f"<p>Head Office has forwarded order <b>#{order.id}</b> "
            f"(customer: {order.customer.name}).</p>"
...
def fan_out_factory_responded(db: Session, order: Order) -> None:
    """Factory -> Customer + HO."""
    ho_users = _users_by_role(db, UserRole.HEAD_OFFICE)
    recipients_users = [order.customer, *ho_users]
    message = f"Factory has responded to order #{order.id}."
    recipients = _notify(db, recipients_users, order, NotificationType.ORDER_RESPONDED, message)
    _send_emails_bg(
        recipients,
        subject=f"[AG Grow] Factory response on order #{order.id}",
        body_html=(
            f"<p>The factory has completed its response to order <b>#{order.id}</b>.</p>"
            f"<p>Please log in to view item availability and notes.</p>"
        ),
    )
